...
export class CirugiaService {
  ...
  crearFichaQuirurgica(lente: CirugiaFicha) {
    return this.http.post<CirugiaFicha[]>(
      this.url_simple + 'cirugia/fichaquirurgica/crear',
      lente
    );
  }
  ...
  getFichaListado() {
    ...
    return this.http.get<CirugiaFicha[]>(
      this.url + '/ficha/ficha/estado/pendiente'
    );
  }
  ...
  getFichaCompleta(cirugia_id: string) {
    return this.http.get<CirugiaFicha[]>(
      this.url + '/ficha/fichacompleta/' + cirugia_id
    );
  }
  ...
  getFichaAnestesia(cirugia_id: string) {
    return this.http.get<CirugiaFicha[]>(
      this.url + '/ficha/lente/' + cirugia_id
    );
  }
  ...
  putFichaMedicoGrupo(cirugiaFichaMedico: CirugiaFichaMedico, id: string) {
    return this.http.put<CirugiaFicha[]>(
      this.url + '/grupomedico/' + id,
      cirugiaFichaMedico
    );
  }

  postLenteFichaMedicoGrupo(CirugiaFichaMedico: any) {
    return this.http.post<CirugiaLente[]>(
      this.url_simple + 'cirugia/grupomedico',
      CirugiaFichaMedico
    );
  }

  getLentesTodosByDate(fecha_desde: string, fecha_hasta: string) {
    return this.http.get<CirugiaLente[]>(
      this.url_simple +
        'stock/lente/by/dates/todos?fecha_desde=' +
        fecha_desde +
        '&fecha_hasta=' +
        fecha_hasta
    );
  ...
  getLentesSinUso(es_baja: string) {
    return this.http.get<CirugiaLente[]>(
      this.url_simple + 'stock/lente/by/todos?es_baja=' + es_baja
    );
  }
  ...
  getLentesCirugiaByDates(fecha_desde: string, fecha_hasta: string) {
    return this.http.get<CirugiaLente[]>(
      this.url_simple +
        'stock/lente/by/dates?fecha_desde=' +
        fecha_desde +
        '&fecha_hasta=' +
        fecha_hasta
    );
  ...
  getLentesCirugiaByDatesAndBaja(
    fecha_desde: string,
    fecha_hasta: string,
    es_baja: string
  ) {
    return this.http.get<CirugiaLente[]>(
      this.url_simple +
        'stock/lente/by/dates/baja?fecha_desde=' +
        fecha_desde +
        '&fecha_hasta=' +
        fecha_hasta +
        '&es_baja=' +
  ...
  putLente(lente: CirugiaLente, id: string) {
    return this.http.put<CirugiaFicha[]>(
      this.url_simple + 'stock/lente/' + id,
      lente
    );
  }
  ...
  postLenteFichaQuirurgica(cirugiaLente: any) {
    return this.http.post<CirugiaLente[]>(
      this.url_simple + 'cirugia/registro/lente',
      cirugiaLente
    );
  }
  ...
  delLenteFichaQuirurgica(lente_id: string, cirugia_id: string) {
    console.log(
      this.url_simple +
        'cirugia/ficha/registro/delete?lente_id=' +
        lente_id +
        '&cirugia_id=' +
        cirugia_id
    );
  ...
  uploadEstudioDatos(estudio: any) {
    return this.http.post(
      this.url_simple + 'multiuploads/estudios/datos',
      estudio
    );
  }
  ...
  getEstudioImagen(id: string) {
    return this.http.get<any[]>(
      this.url_simple + 'multiuploads/estudios/verimagen?id=' + id
    );
  }

  getFichaDerivados(fecha_derivacion: string) {
    return this.http.get<any[]>(
      this.url + '/ficha/derivar/listado?fecha_derivacion=' + fecha_derivacion
    );
  }

  updFichaDerivados(fecha_derivacion: any[], id: string) {
    return this.http.put<any>(
      this.url + '/ficha/derivar/listado/atender/' + id,
      fecha_derivacion
    );
  }
  ...
  getListadoQurifano(fecha_hoy: string) {
    return this.http.get<any[]>(
      this.url + '/listado/quirofano?estado=PENDIENTE&fecha=' + fecha_hoy
    );
  }
  ...
  getListadoQuirofanoByMedico(fecha_hoy: string, medico_id: string) {
    return this.http.get<any[]>(
      this.url +
        '/listado/quirofano/by/medico?estado=PENDIENTE&fecha=' +
        fecha_hoy +
        '&medico_id=' +
        medico_id
    );
  ...
  getListadoQuirofanoByMedicoByPeriodo(
    fecha_desde: string,
    fecha_hasta: string,
    medico_id: string
  ) {
    console.log(
      this.url +
        '/listado/quirofano/by/medico?estado=PENDIENTE&fecha_desde=' +
        fecha_desde +
        '&fecha_hasta=' +
        fecha_hasta +
        '&medico_id=' +
    ...
    return this.http.get<any[]>(
      this.url +
        '/listado/quirofano/by/medico/by/periodo?estado=PENDIENTE&fecha_desde=' +
        fecha_desde +
        '&fecha_hasta=' +
        fecha_hasta +
        '&medico_id=' +
  ...
  getLentesProveedor() {
    return this.http.get<any[]>(this.url_simple + '/stock/lente/proveedor');
  }

  getLentesProveedorById(id: number) {
    return this.http.get<any>(this.url_simple + '/stock/lente/proveedor/' + id);
  }

  putLenteProveedor(parametro: any, id: string) {
    return this.http.put<any>(
      this.url_simple + '/stock/lente/proveedor/' + id,
      parametro
    );
  }
  ...
  setLenteProveedor(parametro: any) {
    return this.http.post<any>(
      this.url_simple + '/stock/lente/proveedor',
      parametro
    );
  }
}
