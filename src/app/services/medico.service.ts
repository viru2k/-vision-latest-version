...
export class MedicoService {
  ...
  getHistoriaClinicaByPaciente(paciente_id: string) {
    return this.http.get<HistoriaClinica[]>(
      this.url_historia_clinica + '/historia/' + paciente_id
    );
  }

  setHistoriaClinicaFicha(registro: HistoriaClinica) {
    return this.http.post<any>(
      this.url_historia_clinica + '/historia/registro/insertar',
      registro
    );
  }
  ...
  updHistoriaClinicaById(registro: HistoriaClinica, id: string) {
    return this.http.put<HistoriaClinica[]>(
      this.url_historia_clinica + '/historia/registro/actualizar' + id,
      registro
    );
  }

  updHistoriaClinicaDni(dniOld: string, dniNew: string) {
    console.log(dniOld, dniNew);
    return this.http.get<any>(
      this.url_historia_clinica +
        '/historia/registro/dni?dniold=' +
        dniOld +
        '&dninew=' +
        dniNew
    );
