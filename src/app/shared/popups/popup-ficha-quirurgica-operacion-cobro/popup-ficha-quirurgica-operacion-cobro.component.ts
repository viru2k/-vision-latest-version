...
export class PopupFichaQuirurgicaOperacionCobroComponent implements OnInit {
  ...
  constructor(
    ...
    public cirugiaService: CirugiaService,
  ...
  asociar() {
    ...
    this.selectedItemCirugia.operacion_cobro_id =
      this.selecteditems[0].operacion_cobro_id;
    try {
      this.cirugiaService
        .updOperacionCobroQurifano(
          this.selectedItemCirugia,
          this.selectedItemCirugia.id
        )
        .subscribe(
          (resp) => {
            console.log(resp);
            ...
            this.throwAlert(
              'success',
              'Se asocio la operación de cobro con éxito',
              '',
              ''
            );
            this.ref.close(resp);
