...
@Component({
  ...
  templateUrl:
    './popup-operacion-cobro-distribucion-multiple-component.component.html',
  styleUrls: [
    './popup-operacion-cobro-distribucion-multiple-component.component.css',
  ],
  providers: [DialogService],
})
export class PopupOperacionCobroDistribucionMultipleComponentComponent
  implements OnInit
{
  ...
  loadRegistroByIdLiquidacion() {
    ...
    try {
      this.miServicio
        .getOperacionCobroRegistrosByLiquidacionNumeroMultiple(this.config.data)
        .subscribe(
          (resp) => {
            if (resp[0]) {
              this.realizarFiltroBusqueda(resp);
  ...
  cargarDistribucion() {
    ...
    ref.onClose.subscribe(
      (PopupDetalleOperacionCobroDistribucionComponent: any) => {
        this.loadRegistroByIdLiquidacion();
