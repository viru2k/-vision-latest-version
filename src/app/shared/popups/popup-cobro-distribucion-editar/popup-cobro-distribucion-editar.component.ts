...
export class PopupCobroDistribucionEditarComponent implements OnInit {
  ...
  async loadList() {
    ...
    this.loading = true;
    try {
      let resp = await this.miServicio.getOperacionCobroDistribucionById(
        this.config.data.operacion_cobro_id,
        this.config.data.estado_liquidacion,
        this.config.data.obra_social_id
      );
      ...
      resp.forEach(
        (element) => {
          if (element['obra_social_id'] != 1) {
            element['distribucion_valor'] =
              (Number(element['distribucion_valor']) * 20) / 80;
            element['distribucion_total'] =
              (Number(element['distribucion_total']) * 20) / 80;
            this.total_suma =
              (Number(resp[0]['operacion_cobro_total_coseguro']) * 20) / 80;
