...
import { LiquidacionDistribucion } from '../../../../../../vision-frontend-v2/src/app/models/liquidacion-distribucion-model';
...
export class PopupDetalleOperacionCobroDistribucionComponent implements OnInit {
  constructor(
    private miServico: MedicoObraSocialService,
  ...
  ngOnInit() {
    ...
    this.liquidacionDistribucion = new LiquidacionDistribucion(
      '',
      '',
      0,
      0,
  ...
  loadList() {
    ...
    this.loading = true;
    try {
      this.miServico.getItemMedicoTodos().subscribe(
        (resp) => {
          let i: number = 0;
          let resultado = resp;
  ...
  GetDistribucionByNumero() {
    ...
    try {
      this.liquidacionService
        .GetDistribucionByNumero(this.config.data.liquidacion_distribucion_id)
        .subscribe(
          (resp) => {
            console.log(resp[0]);
            if (resp[0]) {
  ...
  ActualizarliquidarPracticas() {
    ...
    try {
      this.liquidacionService
        .liquidarOperacionCobroActualizar(this.liquidacionDistribucion)
        .subscribe(
          (resp) => {
            let i: number = 0;
            let resultado = resp;
  ...
  liquidarPracticas() {
    ...
    try {
      this.liquidacionService
        .liquidarOperacionCobro(this.liquidacionDistribucion)
        .subscribe(
          (resp) => {
            let i: number = 0;
            let resultado = resp;
  ...
  guardarDistribucion() {
    ...
    const fecha_distribucion = formatDate(
      new Date(),
      'dd/MM/yyyy HH:mm:ss',
      'en'
    );
    ...
    if (this.medicoOpera) {
      ...
      this.liquidacionDistribucion.medico_opera_porcentaje =
        this.medicoOperaPorcentaje;
      this.liquidacionDistribucion.medico_opera_valor =
        this.medicoOperaDistribucion;
      console.log(this.liquidacionDistribucion);
    }
    if (this.medicoAyuda) {
      this.liquidacionDistribucion.medico_ayuda_id = this.medicoAyuda;
      this.liquidacionDistribucion.medico_ayuda_porcentaje =
        this.medicoAyudaPorcentaje;
      this.liquidacionDistribucion.medico_ayuda_valor =
        this.medicoAyudaDistribucion;
    }
    if (this.medicoAyuda2) {
      this.liquidacionDistribucion.medico_ayuda2_id = this.medicoAyuda2;
      this.liquidacionDistribucion.medico_ayuda2_porcentaje =
        this.medicoAyuda2Porcentaje;
      this.liquidacionDistribucion.medico_ayuda2_valor =
        this.medicoAyuda2Distribucion;
    }
    if (this.medicoClinica) {
      this.liquidacionDistribucion.medico_clinica_id = this.medicoClinica;
      this.liquidacionDistribucion.medico_clinica_porcentaje =
        this.medicoClinicaPorcentaje;
      this.liquidacionDistribucion.medico_clinica_valor =
        this.medicoClinicaDistribucion;
    }
  ...
  console.log(this.filteredItems);*/
    ...
    if (this.config.data.liquidacion_distribucion_id > 0) {
      this.liquidacionDistribucion.id =
        this.config.data.liquidacion_distribucion_id;
  ...
  calcularPorcentaje() {
    this.medicoOperaDistribucion =
      (this.medicoOperaPorcentaje * this.TOTAL_DISTRIBUCION) / 100;
    this.medicoAyudaDistribucion =
      (this.medicoAyudaPorcentaje * this.TOTAL_DISTRIBUCION) / 100;
    this.medicoAyuda2Distribucion =
      (this.medicoAyuda2Porcentaje * this.TOTAL_DISTRIBUCION) / 100;
    this.medicoClinicaDistribucion =
      (this.medicoClinicaPorcentaje * this.TOTAL_DISTRIBUCION) / 100;
  ...
  filterItemsClinica(event) {
    ...
    for (let i = 0; i < this.elementosClinica.length; i++) {
      ...
      if (
        this.elementosClinica[i]['nombre']
          .toLowerCase()
          .indexOf(event.query.toLowerCase()) == 0
      ) {
