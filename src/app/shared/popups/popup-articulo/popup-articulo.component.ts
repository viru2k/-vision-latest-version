...
export class PopupArticuloComponent implements OnInit {
        ...
        (resp) => {
          ...
          if (this.config.data != null) {
            this.elementoAlicuota = this.elementosAlicuota.find(
              (x) => x.iva_id == this.config.data.iva_id
            );
          }
        },
        (error) => {
  ...
  guardar() {
    if (this.config.data != null) {
      ...
      this.updateDataForm.patchValue({
        factura_alicuota_id: this.elementoAlicuota['iva_id'],
      });
  ...
  NuevoArticulo(value: FacturaArticulo) {
    try {
      this.facturacionService.CrearFacturaArticulo(value).subscribe(
        (resp) => {
  ...
  ActualizarArticulo(value: FacturaArticulo) {
    try {
      this.facturacionService.ActualizarFacturaArticulo(value).subscribe(
        (resp) => {
          this.ref.close(value);
        },
        (error) => {
