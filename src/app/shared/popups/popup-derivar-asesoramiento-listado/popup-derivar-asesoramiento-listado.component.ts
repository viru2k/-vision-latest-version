...
import { PopupFichaQuirurgicaLenteComponent } from '../popup-ficha-quirurgica-lente/popup-ficha-quirurgica-lente.component';
import { PopupFichaQuirurgicaEstadoComponent } from '../popup-ficha-quirurgica-estado/popup-ficha-quirurgica-estado.component';
import { PopupFichaQuirurgicaMedicoGrupoComponent } from '../popup-ficha-quirurgica-medico-grupo/popup-ficha-quirurgica-medico-grupo.component';

import { PopupFichaQuirurgicaAnesteciaComponent } from '../popup-ficha-quirurgica-anestecia/popup-ficha-quirurgica-anestecia.component';
...
export class PopupDerivarAsesoramientoListadoComponent implements OnInit {
  ...
  loadList() {
    ...
    this.loading = true;
    try {
      this.miServicio.getFichaDerivados(this._fechaHoy).subscribe(
        (resp) => {
          this.elementos = resp;
          console.log(resp);
          this.loading = false;
  ...
  atendido(event) {
    ...
    this.loading = true;
    try {
      this.miServicio.updFichaDerivados(event, event.id).subscribe(
        (resp) => {
          this.elementos = resp;
          console.log(resp);
          this.loading = false;
  ...
  verLente(selecteditems: any) {
    ...
    ref.onClose.subscribe(
      (PopupFichaQuirurgicaLenteComponent: CirugiaLente) => {
        if (PopupFichaQuirurgicaLenteComponent) {
          ...
          this.FormFicha.patchValue({
            nombre: this.popItemLente.tipo_lente_id,
          });
        }
      }
    );
  ...
  verHistoriaClinica(selecteditems) {
    ...
    try {
      this.medicoService
        .getHistoriaClinicaByPaciente(this.selecteditem.paciente_dni)
        .subscribe(
          (resp) => {
  ...
  verAnestesia(selecteditems) {
    ...
    ref.onClose.subscribe(
      (PopupFichaQuirurgicaAnesteciaComponent: CirugiaFichaAnestesia) => {
  ...
  verMedico(selecteditems) {
    ...
    ref.onClose.subscribe(
      (PopupFichaQuirurgicaMedicoGrupoComponent: CirugiaFichaMedico) => {
        this.loadList();
      }
    );
  }
  ...
  verListadoCirugiaAtencion(selecteditems) {
    ...
    ref.onClose.subscribe(
      (PopupDerivarAsesoramientoListadoComponent: CirugiaFicha) => {
        // this.loadList();
      }
    );
  }
