...
export class PopupOperacionCobroDistribucionDetalleComponent implements OnInit {
  ...
  constructor(
    ...
    this.cols = [
      ...
      {
        field: 'medico_ayuda2_valor',
        header: 'Médico ayuda 2 $',
        width: '10%',
      },
      {
        field: 'medico_ayuda2_porcentaje',
        header: 'Médico ayuda 2 %',
        width: '6%',
      },
  ...
  sumarValores(vals: any) {
    ...
    for (i = 0; i < vals.length; i++) {
      this.TOTAL_OPERA =
        this.TOTAL_OPERA + Number(vals[i]['medico_opera_valor']);
      this.TOTAL_AYUDA =
        this.TOTAL_AYUDA + Number(vals[i]['medico_ayuda_valor']);
      this.TOTAL_AYUDA2 =
        this.TOTAL_AYUDA2 + Number(vals[i]['medico_ayuda2_valor']);
      this.TOTAL_CLINICA =
        this.TOTAL_CLINICA + Number(vals[i]['medico_clinica_valor']);
  ...
  getMedicosFacturan() {
    this.loading = true;
    try {
      this.medicoService.getItems().subscribe(
        (resp) => {
  ...
  public exportarExcel() {
    ...
    this.elementos.forEach((element) => {
      if (
        element.medico_opera === this.elementoMedicos.apellido ||
        element.medico_ayuda === this.elementoMedicos.apellido ||
        element.medico_ayuda2 === this.elementoMedicos.apellido ||
        element.medico_clinica === this.elementoMedicos.apellido
      ) {
        this.distribucionMedico = new DistribucionMedico(
    ...
    if (this.config.data.consulta === 'detalle') {
      if (this.checked) {
        ...
        this.elementos.forEach((elem) => {
          ...
          if (
            elem.medico_opera === this.elementoMedicos.apellido ||
            elem.medico_ayuda === this.elementoMedicos.apellido ||
            elem.medico_ayuda2 === this.elementoMedicos.apellido ||
            elem.medico_clinica === this.elementoMedicos.apellido
          ) {
            if (esPrimero) {
              esPrimero = false;
      ...
      if (selectedImpresion) {
        ...
        doc.setFontSize(10);
        doc.text(
          'DISTRIBUCION A ' + this.elementoMedicos.apellido,
          pageWidth / 2,
          10,
          null,
          null,
        ...
        doc.text(
          ...
          finalY + 8,
          'TOTAL OPERA : ' +
            this.cp.transform(_total_opera, '', 'symbol-narrow', '1.2-2')
        );
        doc.text(
          65,
          finalY + 8,
          'TOTAL AYUDA : ' +
            this.cp.transform(_total_ayuda, '', 'symbol-narrow', '1.2-2')
        );
        doc.text(
          115,
          finalY + 8,
          'TOTAL AYUDA 2 : ' +
            this.cp.transform(_total_ayuda2, '', 'symbol-narrow', '1.2-2')
        );
        doc.text(
          165,
          finalY + 8,
          'TOTAL CLINICA : ' +
            this.cp.transform(_total_clinica, '', 'symbol-narrow', '1.2-2')
        );
        doc.text(
          pageWidth - 50,
          finalY + 8,
          'TOTAL A PAGAR : ' +
            this.cp.transform(
              _total_opera + _total_ayuda + _total_ayuda2 + _total_clinica,
              '',
              'symbol-narrow',
              '1.2-2'
            )
      ...
      if (this.checked) {
        ...
        this.elementos.forEach((element) => {
          try {
            element.fecha_cobro = formatDate(
              element.fecha_cobro,
              'dd/MM/yyyy',
              'en'
            );
          ...
          if (element.medico_clinica === this.elementoMedicos.apellido) {
            _total_clinica =
              _total_clinica + Number(element.medico_clinica_valor);
          }
        });
      } else {
        selectedImpresion = this.selecteditems;
