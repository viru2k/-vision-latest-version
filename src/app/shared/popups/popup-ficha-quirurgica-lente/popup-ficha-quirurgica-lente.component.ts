...
export class PopupFichaQuirurgicaLenteComponent implements OnInit {
  ...
  loadList() {
    try {
      this.miServicio.getFichaLente(this.config.data.id).subscribe(
        (resp) => {
          this.elementos = resp;
          console.log(resp);
          this.loading = false;
  ...
  quitar(event) {
    ...
    try {
      this.miServicio
        .delLenteFichaQuirurgica(event.lente_id, event.cirugia_id)
        .subscribe(
          (resp) => {
            //    this.elementos = resp;
            console.log(resp);
            ...
            this.throwAlert(
              'success',
              'Se elimino el lente de la ficha con éxito',
              '',
              ''
            );
          },
