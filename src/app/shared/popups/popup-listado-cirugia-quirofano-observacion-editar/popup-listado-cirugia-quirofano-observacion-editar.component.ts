...
@Component({
  ...
  templateUrl:
    './popup-listado-cirugia-quirofano-observacion-editar.component.html',
  styleUrls: [
    './popup-listado-cirugia-quirofano-observacion-editar.component.css',
  ],
})
export class PopupListadoCirugiaQuirofanoObservacionEditarComponent
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    if (this._es_nuevo == 'NO') {
      ...
      this.updateDataForm.patchValue({
        fecha_hora: this.config.data.fecha_hora,
      });
      this.updateDataForm.patchValue({ orden: this.config.data.orden });
      this.updateDataForm.patchValue({
        cirugia_ficha_id: this.config.data.cirugia_ficha_id,
      });
      ...
      this.updateDataForm.patchValue({
        cirugia_practica: this.config.data.cirugia_practica,
      });
      this.updateDataForm.patchValue({
        quirofano_observacion: this.config.data.quirofano_observacion,
      });
      this.updateDataForm.patchValue({
        tiene_observacion: this.config.data.tiene_observacion,
      });
    } else {
  ...
  actualizarDatos() {
    if (this.updateDataForm.value.es_nuevo === 'NO') {
      ...
      try {
        this.miServicio
          .updListadoQurifano(this.config.data, this.updateDataForm.value.id)
          .subscribe(
            (resp) => {
              this.throwAlert(
                'success',
      ...
      try {
        this.miServicio
          .postListadoQurifano(this.config.data, this.updateDataForm.value.id)
          .subscribe(
            (resp) => {
              this.throwAlert(
                'success',
