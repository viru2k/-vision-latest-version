...
export class PopupOperacionCobroRegistroBuscarTodosComponent implements OnInit {
  ...
  loadList() {
    ...
    try {
      this.miServicio
        .getOperacionCobroRegistrosByPaciente(this.elemento['id'])
        .subscribe(
          (resp) => {
            if (resp[0]) {
  ...
  operacionCobro() {
    try {
      ...
      this.miServicio
        .putOperacionCobroRegistroAnular(this.elementos[0], this.textoBusqueda)
        .subscribe(
          (resp) => {
            console.log(resp);
            this.elementos = resp;
            ...
            this.throwAlert(
              'success',
              'Se anuló el registro con éxito',
              '',
              ''
            );
          },
