...
export class PopupPresentacionEditarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.config.data.fecha_desde = new Date(
      _fecha_desde.getTime() + _fecha_desde.getTimezoneOffset() * 60 * 1000
    );

    let _fecha_hasta: Date = new Date(this.config.data.fecha_hasta);
    this.config.data.fecha_hasta = new Date(
      _fecha_hasta.getTime() + _fecha_hasta.getTimezoneOffset() * 60 * 1000
    );

    let _numero: Date = new Date(this.config.data.numero);
    this.config.data.numero = new Date(
      _numero.getTime() + _numero.getTimezoneOffset() * 60 * 1000
    );
  ...
  actualizarFechaDesde(event) {
    ...
    this.updateDataForm.patchValue({
      fecha_desde: this.config.data.fecha_desde,
    });
  }
  ...
  actualizarFechaHasta(event) {
    ...
    this.updateDataForm.patchValue({
      fecha_hasta: this.config.data.fecha_hasta,
    });
  }
