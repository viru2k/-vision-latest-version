...
export class PopupFichaQuirurgicaEstadoComponent implements OnInit {
  ...
  constructor(
    public miServico: CirugiaService,
  ...
  ngOnInit() {
    ...
    this.DateForm = new FormGroup({
      ...
      fecha_inicio_acto_quirurgico: new FormControl(
        this.fecha_inicio_acto_quirurgico,
        Validators.required
      ),
      fecha_fin_acto_quirurgico: new FormControl(
        this.fecha_fin_acto_quirurgico,
        Validators.required
      ),
  ...
  guardarDatos() {
    ...
    this.DateForm.patchValue({
      fecha_inicio_acto_quirurgico: formatDate(
        this.fecha_inicio_acto_quirurgico,
        'dd/MM/yyyy HH:mm',
        'en'
      ),
    ...
    this.DateForm.patchValue({
      fecha_fin_acto_quirurgico: formatDate(
        this.fecha_fin_acto_quirurgico,
        'dd/MM/yyyy HH:mm',
        'en'
      ),
    ...
    this.DateForm.patchValue({
      fecha_alta: formatDate(this.fecha_alta, 'dd/MM/yyyy HH:mm', 'en'),
    });
    let userData = JSON.parse(localStorage.getItem('userData'));
    this.DateForm.patchValue({
      estado_cirugia_id: this.selectedCirugia['code'],
    });
    ...
    try {
      this.miServico
        .updEstadoQurifano(this.DateForm.value, this.config.data.id)
        .subscribe(
          (resp) => {
            this.loading = false;
            this.ref.close(resp);
