...
export class PopupHistoriaClinicaRegistroNuevoComponent implements OnInit {
  ...
  guardarHistoria() {
    ...
    this.dataForm.patchValue({
      nombreyapellido: this.elemento_temp.nombreyapellido,
    });
    this.dataForm.patchValue({
      numero_afiliado: this.elemento.numero_afiliado,
    });
    this.dataForm.patchValue({ domicilio: this.elemento.domicilio });
    this.dataForm.patchValue({ edad: this.elemento.edad });
    this.dataForm.patchValue({
      obra_social_nombre: this.elemento.obra_social_nombre,
    });
    this.dataForm.patchValue({
      paciente_nombre: this.elemento.paciente_nombre,
    });
    this.dataForm.patchValue({ plan: this.elemento.plan });

    try {
      this.miServicio.setHistoriaClinicaFicha(this.dataForm.value).subscribe(
        (resp) => {
          swal({
            toast: false,
            type: 'success',
  ...
  actualizarLEJOS_OD_P() {
    this.dataForm.patchValue({
      LEJOS_OD_P: this.cp.transform(this.dataForm.value.LEJOS_OD_P, '1.2-2'),
    });
  }
  actualizarLEJOS_OD_B() {
    this.dataForm.patchValue({
      LEJOS_OD_B: this.cp.transform(this.dataForm.value.LEJOS_OD_B, '1.2-2'),
    });
  }
  ...
  actualizarLEJOS_OI_P() {
    this.dataForm.patchValue({
      LEJOS_OI_P: this.cp.transform(this.dataForm.value.LEJOS_OI_P, '1.2-2'),
    });
  }
  actualizarLEJOS_OI_B() {
    this.dataForm.patchValue({
      LEJOS_OI_B: this.cp.transform(this.dataForm.value.LEJOS_OI_B, '1.2-2'),
    });
  }
  ...
  verHistoria() {
    ...
    ref.onClose.subscribe(
      (PopupHistoriaClinicaListaConsultaComponent: any) => {}
    );
  }
  ...
  imprimirRecetaEscrita() {
    ...
    doc.text(
      'Obra social: ' + this.elemento.obra_social_nombre,
      10 + x,
      15 + y
    );
    ...
    if (this.elemento.paciente_obra_social_id == '1') {
      ...
      if (this.elemento.plan === '0') {
        doc.text(
          'Numero: ' +
            this.elemento.numero_afiliado +
            '/' +
            this.elemento.barra_afiliado,
          10 + x,
          20 + y
        );
      } else {
  ...
  imprimirReceta() {
    ...
    /** EJES  */
    if (
      this.dataForm.value.CERCA_OI_2 === null ||
      this.dataForm.value.CERCA_OI_2 === ''
    ) {
    } else {
      CERCA_OI_2 = this.dataForm.value.CERCA_OI_2 + '°';
    }
    if (
      this.dataForm.value.CERCA_OD_2 === null ||
      this.dataForm.value.CERCA_OD_2 === ''
    ) {
    } else {
    ...
    if (
      this.dataForm.value.LEJOS_OD_2 === null ||
      this.dataForm.value.LEJOS_OD_2 === ''
    ) {
    } else {
    ...
    if (
      this.dataForm.value.LEJOS_OI_2 === null ||
      this.dataForm.value.LEJOS_OI_2 === ''
    ) {
    } else {
