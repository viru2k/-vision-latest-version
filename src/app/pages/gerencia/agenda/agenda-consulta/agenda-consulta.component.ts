...
export class AgendaConsultaComponent implements OnInit {
  ...
  public exportarExcelDetallado() {
    ...
    this.selecteditems.forEach((element) => {
      ...
      seleccionados['obra_social_nombre'] =
  ...
  loadListByMedico() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        this.miServico
          .getHorarioTurnoTodosSinEstado(this.popItemAgenda)
          .subscribe(
            (resp) => {
              if (resp[0]) {
  ...
  loadListByDates() {
    ...
    try {
      this.miServico.getHorarioTurnoTodosSinEstadoByDates(td, th).subscribe(
        (resp) => {
          //  console.log(resp);
  ...
  loadListByDatesOC() {
    ...
    try {
      this.miServico
        .getHorarioTurnoTodosSinEstadoByDatesGerencia(td, th)
        .subscribe(
          (resp) => {
            //  console.log(resp);
  ...
  onmyChange(event) {
    ...
    this.table.filter(
      event.itemValue.value,
      this.agendaTurno['nombreyapellido'],
      'in'
    );
  }
