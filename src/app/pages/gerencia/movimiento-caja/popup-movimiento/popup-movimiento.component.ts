        ...
        (resp) => {
          ...
          if (resp[0]) {
            resp.forEach((element) => {
              resp[i].cuenta_nombre =
                element.cuenta_nombre + ' ' + element.movimiento_tipo;
              i++;
            });
              ...
              this.updateDataForm
                ...
                .setValue(
                  this.elementoCuenta.find(
                    (elem) =>
                      elem.cuenta_nombre ===
                      this.config.data.cuenta_nombre +
                        ' ' +
                        this.config.data.movimiento_tipo
                  )
                );
            }
          }
  ...
  buscarPaciente() {
    ...
    ref.onClose.subscribe((PopupMovimientoFindPacienteCobroComponent: any) => {
      if (PopupMovimientoFindPacienteCobroComponent) {
        ...
        this.updateDataForm.patchValue({
          paciente_id: PopupMovimientoFindPacienteCobroComponent.id,
        });
        this.updateDataForm.patchValue({
          nombreyapellido_paciente:
            PopupMovimientoFindPacienteCobroComponent.apellido +
            ' ' +
            PopupMovimientoFindPacienteCobroComponent.nombre,
        });
      }
    });
  }
  ...
  buscarProveedor() {
    ...
    ref.onClose.subscribe((PopupProveedorFindComponent: any) => {
      if (PopupProveedorFindComponent) {
        ...
        this.updateDataForm.patchValue({
          nombreyapellido_proveedor:
            PopupProveedorFindComponent.proveedor_nombre,
        });
        this.updateDataForm.patchValue({
          proveedor_id: PopupProveedorFindComponent.id,
        });
      }
    });
  }
  ...
  buscarDistribucionCobro() {
    ...
    ref.onClose.subscribe((PopupMovimientoFindPacienteCobroComponent: any) => {
      if (PopupMovimientoFindPacienteCobroComponent) {
        console.log(PopupMovimientoFindPacienteCobroComponent);
        this.updateDataForm.patchValue({
          paciente_id: PopupMovimientoFindPacienteCobroComponent.paciente_id,
        });
        this.updateDataForm.patchValue({
          nombreyapellido_paciente:
            PopupMovimientoFindPacienteCobroComponent.apellido +
        ...
        this.updateDataForm.patchValue({
          liq_liquidacion_distribucion_id:
            PopupMovimientoFindPacienteCobroComponent.liquidacion_distribucion_id,
        });
  ...
  buscarFactura() {
    ...
    ref.onClose.subscribe((PopupMovimientoFindFacturaComponent: any) => {
      if (PopupMovimientoFindFacturaComponent) {
        ...
        this.updateDataForm.patchValue({
          comprobante_numero:
            PopupMovimientoFindFacturaComponent.descripcion +
            ' ' +
            PopupMovimientoFindFacturaComponent.comprobante_codigo +
            '-' +
            PopupMovimientoFindFacturaComponent.factura_numero,
        });
        this.updateDataForm.patchValue({
          factura_encabezado_id: PopupMovimientoFindFacturaComponent.id,
        });
    ...
    if (this.esEditar) {
      try {
        this.movimientoCajaService
          .putMovimientoCaja(this.updateDataForm.value, this.config.data.id)
          .subscribe(
            (resp) => {
              this.alertServiceService.throwAlert(
                'success',
      ...
      try {
        this.movimientoCajaService
          .setMovimientoCaja(this.updateDataForm.value)
          .subscribe(
            (resp) => {
              this.alertServiceService.throwAlert(
                'success',
