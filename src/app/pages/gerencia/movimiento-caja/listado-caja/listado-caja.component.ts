...
export class ListadoCajaComponent implements OnInit {
            ...
            if (resp[0]) {
              resp.forEach((element) => {
                if (element.proveedor_nombre === null) {
                  ...
                  element.nombreyapellido_paciente =
                    element.paciente_nombre + element.proveedor_nombre;
                }

                if (element.paciente_nombre === null) {
                  element.paciente_nombre = '';
                  element.nombreyapellido_proveedor =
                    element.paciente_nombre + element.proveedor_nombre;
                }
                element.proveedor_nombre =
                  element.paciente_nombre + element.proveedor_nombre;
  ...
  generarPdf() {
    let i = 0;
    this.selecteditems.forEach((element) => {
      this.selecteditems[i].fecha_carga = formatDate(
        element.fecha_carga,
        'dd/MM/yyyy',
        'en'
      );
    ...
    doc.setFontSize(8);
    doc.text(
      'Desde : ' + _fechaDesde_imp + ' Hasta ' + _fechaHasta_imp,
      pageWidth / 2,
      28,
      null,
      null,
