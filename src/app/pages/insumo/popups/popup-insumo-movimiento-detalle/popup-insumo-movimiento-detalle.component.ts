...
export class PopupInsumoMovimientoDetalleComponent implements OnInit {
  ...
  constructor(
    ...
    this.cols = [
      ...
      {
        field: 'insumo_stock_movimiento_fecha_ingreso',
        header: 'Movimien.',
        width: '20%',
      },
      ...
      {
        field: 'insumo_stock_movimiento_cantidad_usada',
        header: 'C Usada',
        width: '12%',
      },
      {
        field: 'insumo_stock_movimiento_cantidad_existente',
        header: 'Existencia',
        width: '12%',
      },
  ...
  loadList() {
    ...
    try {
      this.miServicio
        .getInsumoStockMovimiento(this.config.data['id'])
        .subscribe(
          (resp) => {
            this.elementos = resp;
            console.log(resp);
