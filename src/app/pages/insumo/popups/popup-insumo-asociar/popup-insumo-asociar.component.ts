...
export class PopupInsumoAsociarComponent implements OnInit {
  ...
  constructor(
    private miServicio: InsumoService,
  ...
  ngOnInit() {
    ...
    this.updateDataForm.patchValue({
      insumo_id: this.config.data['insumo_id'],
    });
    ...
    this.updateDataForm.patchValue({
      cantidad_existente: this.config.data['cantidad_existente'],
    });
    this.updateDataForm.patchValue({
      cantidad_original: this.config.data['cantidad_original'],
    });
    this.updateDataForm.patchValue({
      cantidad_usada_insumo: this.config.data['cantidad_usada'],
    });
  ...
  guardarDatos() {
    ...
    if (
      this.updateDataForm.value.cantidad_existente <
      this.updateDataForm.value.cantidad_usada
    ) {
      swal({
        type: 'warning',
      ...
      try {
        this.miServicio
          .crearInsumoStockMovimiento(this.updateDataForm.value)
          .subscribe(
            (resp) => {
              console.log(resp);
              this.loading = false;
