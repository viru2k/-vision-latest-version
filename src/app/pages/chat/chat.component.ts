...
export class ChatComponent implements OnInit, AfterViewInit {
    ...
    this.documentService.getMessages().subscribe((message: string) => {
      ...
      if (this.usuarioChat) {
        if (message['chat_sesion_id'] === this.usuarioChat.chat_sesion_id) {
          ...
          const valor = this.lista_usuarios.findIndex(
            (x) => x.chat_sesion_id === message['chat_sesion_id']
          );
          // console.log(valor);
  ...
  verUsuariosDetalleByGrupo(sesion_id: string) {
    ...
    if (this.userData['id']) {
      try {
        this.chatService.getGrupoDetalleUsuarios(sesion_id).subscribe(
          (resp) => {
  ...
  loadListaUsuario() {
    ...
    if (this.userData['id']) {
      try {
        this.chatService.getSesionListByUsuario(this.userData['id']).subscribe(
          (resp) => {
  ...
  loadChat(e: any) {
    ...
    const valor = this.lista_usuarios.findIndex(
      (x) => x.chat_sesion_id === e['chat_sesion_id']
    );
    //console.log(valor);
    this.lista_usuarios[valor]['estado'] = 'LEIDO';
    try {
      this.chatService
        .getChatBySesion(e.id, e.chat_sesion_id, e.grupo_nombre, this.limite)
        .subscribe(
          (resp) => {
  ...
  enviarTexto() {
    if (this.textoAenviar !== '') {
      ...
      const mensaje = new ChatRenglon(
        this.usuarioChat.chat_sesion_id,
        this.userData['id'],
        this.textoAenviar,
        'NO',
        '',
      ...
      this.loading_chat = true;
      try {
        this.chatService.insertarRenglonChat(mensaje).subscribe(
          (resp) => {
            console.log(resp);
            this.loading = false;
            this.loadDetalle = '';
