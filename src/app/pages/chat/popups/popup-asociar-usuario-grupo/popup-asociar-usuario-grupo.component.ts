...
export class PopupAsociarUsuarioGrupoComponent implements OnInit {
  ...
  constructor(
    ...
    this.cols = [{ field: 'nombreyapellido', header: 'Usuario', width: '20%' }];
    this.cols_grupo = [
    ...
    try {
      this.chatService
        .asociarUsuarioGrupo(
          userData['id'],
          element.id,
          this.selectedItem_grupo['grupo_nombre'],
          this.selectedItem_grupo['id']
        )
        .subscribe(
          (resp) => {
            console.log(this.elementos);
  ...
  crearGrupo() {
    if (this.grupo != '') {
      this.loading = true;
      try {
        this.chatService.crearSesionListadoGrupo(this.grupo).subscribe(
          (resp) => {
            this.elementos = resp;
            console.log(this.elementos);
            this.loading = false;
  ...
  borrar(element) {
    ...
    try {
      this.chatService
        .destroyUsuarioGrupoSesion(
          element.value.chat_sesion_id,
          element.value.usuario_id
        )
        .subscribe(
          (resp) => {
            // this.detalleUsuarios = resp;
