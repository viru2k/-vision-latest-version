...
export class AgendaAtencionMedicoComponent implements OnInit {
  ...
  pacienteDerivado(event: any) {
    ...
    try {
      this.miServico
        .pacienteDerivado(
          this.selectedagendaTurno,
          this.selectedagendaTurno.agenda_dia_horario_atencion_id
        )
        .subscribe(
          (resp) => {
  ...
  async editarRegistro(cond: string) {
    ...
    if (cond == 'historia') {
      ...
      this.router.navigate(['/medico/historiaclinica'], {
  ...
  loadListTodosTurnos() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        this.miServico
          .getHorarioTurnoMedicoSinEstado(this.popItemAgenda)
          .subscribe(
            (resp) => {
              if (resp[0]) {
                this.agendaTurno = resp;
