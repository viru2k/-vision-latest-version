...
export class PopupMedicoEditComponent implements OnInit {
  ...
  obtenerCondicionIva(event: any) {
    ...
    this.updateDataForm.patchValue({
      categoria_iva_id: this.elementoCondicionIva['id'],
    });
  }
  ...
  obtenerTipoDocumento(event: any) {
    ...
    this.updateDataForm.patchValue({
      factura_documento_comprador_id: this.elementoDocumento['id'],
    });
  }
  ...
  obtenerComprobante(event: any) {
    ...
    this.updateDataForm.patchValue({
      factura_comprobante_id: this.elementoComprobante['id'],
    });
  }
      ...
      this.facturacionService.CategoriaIva().subscribe(
        (resp) => {
          ...
          this.elementoCondicionIva = this.elementosCondicionIva.find(
            (x) => x.id == this.config.data['categoria_iva_id']
          );
          console.log(resp);
        },
        (error) => {
      ...
      this.facturacionService.Comprobante().subscribe(
        (resp) => {
          ...
          resultado.forEach((element) => {
            resp[i]['descripcion'] =
              resp[i]['descripcion'] + ' - ' + resp[i]['letra'];
            i++;
          });
          ...
          this.elementoComprobante = this.elementosComprobante.find(
            (x) => x.id == this.config.data['factura_comprobante_id']
          );
          this.updateDataForm.patchValue({
            factura_comprobante_id: this.elementoComprobante['id'],
          });
      ...
      this.facturacionService.Documento().subscribe(
        (resp) => {
          ...
          this.elementoDocumento = this.elementosDocumento.find(
            (x) => x.id == this.config.data['factura_documento_comprador_id']
          );
        },
        (error) => {
          // error path
      ...
      this.facturacionService.PtoVta().subscribe(
        (resp) => {
          ...
          this.elementoPtoVta = this.elementosPtoVta.find(
            (x) => x.id == this.config.data['punto_vta_id']
          );
