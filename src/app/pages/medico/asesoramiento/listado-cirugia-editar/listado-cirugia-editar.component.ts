...
export class ListadoCirugiaEditarComponent implements OnInit {
  ...
  constructor(
    ...
    this.cols = [
      ...
      {
        field: 'obra_social_nombre',
        header: 'Obra social / Coseguro',
        width: '20%',
      },
      ...
      {
        field: 'usuario_medico_anestesista_nombre',
        header: 'Anestesista',
        width: '12%',
      },
  ...
  verObservacion(
    ...
    event: CirugiaFichaListadoQuirofano
  ) {
    if (event) {
  ...
  loadList() {
    ...
    try {
      this.miServicio.getListadoQurifano(_fechaDesde).subscribe(
        (resp) => {
          let i: number = 0;
          let resultado = resp;
          resultado.forEach((element) => {
            resp[i]['obra_social_nombre'] =
              resp[i]['obra_social_nombre'] +
              ' / ' +
              resp[i]['coseguro_nombre'];
  ...
  loadListByMedico() {
    ...
    try {
      this.miServicio
        .getListadoQuirofanoByMedico(_fechaDesde, userData['id'])
        .subscribe(
          (resp) => {
            let i: number = 0;
            let resultado = resp;
  ...
  editar(selecteditems: any) {
    ...
    ref.onClose.subscribe(
      (PopupListadoCirugiaQuirofanoObservacionEditarComponent: any) => {
        this.loadList();
      }
    );
  }
  ...
  darTurno() {
    this.router.navigate(['/recepcion/turnos'], {
      state: { paciente: this.selecteditem },
    });
  }
  ...
  generarPdfListadoMedico() {
    ...
    if (this.selecteditems) {
      ...
      doc.setFontSize(9);
      doc.text(
        'Usuario confecciono: ' + userData['nombreyapellido'],
        60,
        20,
        null,
        null,
      ...
      doc.autoTable(this.columns, this.elementos, {
        ...
        columnStyles: {
          cirugia_ficha_id: { columnWidth: 10 },
          orden: { columnWidth: 10 },
          hora: { columnWidth: 10 },
          paciente_apellido: { columnWidth: 20 },
          paciente_nombre: { columnWidth: 20 },
          obra_social_nombre: { columnWidth: 25 },
          ojo: { columnWidth: 8 },
          dioptria: { columnWidth: 8 },
          paciente_dni: { columnWidth: 15 },
          lote: { columnWidth: 16 },
        },
