...
export class UsuarioEditarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.config.data) {
      ...
      this.updateDataForm.patchValue({
        nombreyapellido: this.config.data.nombreyapellido,
      });
  ...
  crearUsuario() {
    ...
    try {
      this.userService.setUser(this.updateDataForm.value).subscribe(
        (resp) => {
          this.loading = false;
          console.log(resp);
          this.ref.close(resp);
  ...
  modificarUsuario() {
    ...
    try {
      this.userService
        .updUser(this.updateDataForm.value.id, this.updateDataForm.value)
        .subscribe(
          (resp) => {
            this.loading = false;
            console.log(resp);
