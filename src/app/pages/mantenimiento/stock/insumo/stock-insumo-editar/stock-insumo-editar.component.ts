...
export class StockInsumoEditarComponent implements OnInit {
  ...
  guardar() {
    if (this.config.data) {
      ...
      this.popItem.insumo_descripcion =
        this.updateDataForm.value.insumo_descripcion;
  ...
  NuevoInsumo(value: Insumo) {
    console.log(value);
    try {
      this.insumoService.crearInsumo(value).subscribe(
        (resp) => {
  ...
  ActualizarInsumo(value: Insumo) {
    try {
      this.insumoService.actualizarInsumo(value, value.id).subscribe(
        (resp) => {
          this.ref.close(value);
        },
        (error) => {
