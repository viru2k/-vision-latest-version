...
export class PopupMovimientoCuentaEditarComponent implements OnInit {
  ...
  elementos: any;
  categories: any[] = [
    { name: 'INGRESO', key: 'INGRESO' },
    { name: 'EGRESO', key: 'EGRESO' },
  ];
  ...
  ngOnInit() {
    ...
    if (this.config.data) {
      ...
      this.updateDataForm.patchValue({
        cuenta_nombre: this.config.data.cuenta_nombre,
      });
      this.updateDataForm.patchValue({
        movimiento_tipo: this.config.data.movimiento_tipo,
      });
      //this.uda
  ...
  nuevaUnidad() {
    ...
    try {
      this.movimientoCajaService
        .setMovimientoCuenta(this.updateDataForm.value)
        .subscribe(
          (resp) => {
            this.loading = false;
            console.log(resp);
