...
export class AgendaBloqueoComponent implements OnInit {
  ...
  loadHorarios() {
    ...
    if (this._fechaHoy != '') {
      try {
        this.miServico
          .getAgendaBloqueoByMedicoAndDiaTodoEstado(
      ...
      try {
        this.miServico
          .bloquearTurno(
            this._fechaHoy,
            res.usuario_id,
            res.agenda_usuario_dia_horario_id
          )
          .subscribe(
            (resp) => {
              this.loadHorarios();
  ...
  bloquearPeriodo(res) {
    ...
    if (this.selectedItemUsuario != undefined) {
      for (i = 0; i < res.length; i++) {
        this.fechaBloqueo = new AgendaDiaBloqueo(
          '0',
          res[i],
          this.selectedItemUsuario.id
        );
      ...
      this.loading = true;
      try {
        this.miServico.bloquearAgendaTurno(this.fechasBloqueo).subscribe(
          (resp) => {
            // this.loadHorarios();
            this.throwAlert(
              'success',
              'Se bloquearon las fechas para el usuario: ' +
                this.selectedItemUsuario.nombreyapellido,
              '',
              ''
            );
            // this.agendaTurno = resp;
