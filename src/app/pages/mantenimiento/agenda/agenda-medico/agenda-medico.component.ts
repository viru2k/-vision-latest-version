...
export class AgendaMedicoComponent implements OnInit {
  ...
  constructor(
    ...
    this.cols = [{ field: 'nombreyapellido', header: 'Usuario' }];

    this.colsDias = [{ field: 'dia_nombre', header: 'Dia' }];
  ...
  loadDias() {
    ...
    this.loading = true;
    try {
      this.miServico.getDias().subscribe(
        (resp) => {
          this.dias = resp;
          this.loading = false;
          console.log(resp);
  ...
  loadHorasPeriodo() {
    this.loading = true;
    try {
      this.miServico.getHorario().subscribe(
        (resp) => {
          this.horario = resp;
          this.loading = false;
          console.log(resp);
  ...
  loadHorarios() {
    ...
    try {
      this.miServico
        .getAgendaByMedicoTodos(
          this.selectedItemUsuario.id,
          this.selectedItemDia.id,
          'S'
        )
        .subscribe(
          (resp) => {
  ...
  updateEstado() {
    ...
    try {
      this.miServico
        .putAgendaDeshabilitar(
          this.selectedItemAgenda.id,
          this.selectedItemAgenda
        )
        .subscribe(
          (resp) => {
            this.showToast('exito', 'Renglon actualizado', 'Guardado correcto');
  ...
  crearAgendaDia() {
    // this.loading = true;
    try {
      this.miServico.crearAgendaMedico(this.agendaMedicoHorarioDia).subscribe(
        (resp) => {
          this.showToast('exito', 'Renglon actualizado', 'Guardado correcto');
          console.log(resp);
        },
  ...
  guardarDatos() {
    ...
    this.agendaMedicoHorarioDia = new AgendaMedicoHorarioDia(
      '0',
      this.selectedItemUsuario.id,
      this.selectedItemDia.id,
      this.selectedItemHorario.id
    );
  ...
  onRowUsuario() {
    ...
    if (
      this.selectedItemDia != null &&
      this.selectedItemUsuario != null &&
      this.selectedItemHorario != null
    ) {
