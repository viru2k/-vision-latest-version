...
export class ConvenioComponent implements OnInit {
  ...
  constructor(
    ...
    private miServicoOs: ObraSocialService,
  ...
  showDialogToUpdate(event) {
    ...
    this.popItem = new Convenio(
      event.id,
      event.valor,
      event.obra_social_id,
      event.obra_social_nombre,
      '',
      '',
      0,
      event.es_habilitada,
      event.pmo_id,
      event.codigo,
      event.pmo_descripcion,
      event.complejidad,
      event.es_habilitado,
      event.es_coseguro,
      event.tiene_distribucion
    );
    let data: any;
    data = this.popItem;
    const ref = this.dialogService.open(EditConvenioComponent, {
  ...
  insertarPracticas() {
    ...
    this.loading = true;
    try {
      this.miServicoOs.insertarCoseguro().subscribe(
        (resp) => {
          // this.elementos = resp;
          this.showToast(
            'exito',
            'Registros insertados',
            'Exito al insertar registros'
          );
          this.loading = false;
          this.loadList();
          console.log(resp);
  ...
  ActualizarValores() {
    ...
    this.loading = true;
    try {
      this.miServicoOs.actualizarCoseguro().subscribe(
        (resp) => {
          this.showToast(
            'exito',
