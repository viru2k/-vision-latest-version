...
export class AgendaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.documentService.getMessages().subscribe((message: string) => {
      console.log(message);
          ...
          (resp) => {
            if (resp[0]) {
              ...
              resultado.forEach((element) => {
                resp[i]['telefono_fijo'] =
                  resp[i]['telefono_cel'] + ' / ' + resp[i]['telefono_fijo'];
                resp[i]['edad'] = String(
                  new Date().getFullYear() -
                    new Date(resp[i]['paciente_fecha_nacimiento']).getFullYear()
                );
                i++;
              });
  ...
  generarComprobante() {
    ...
    let fecha = formatDate(
      this.selectedagendaTurno['fecha_turno'],
      'dd/MM/yyyy',
      'en'
    );
    var doc = new jsPDF();
    ...
    doc.text(
      ...
      75,
      'PACIENTE :' +
        this.selectedagendaTurno['paciente_apellido'] +
        ' ' +
        this.selectedagendaTurno['paciente_nombre']
    );
