...
export class AgendaRecepcionComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.cols = [
      ...
      {
        field: 'paciente_obra_social_nombre',
        header: 'Obra social',
        width: '8%',
      },
  ...
  ngOnInit() {
    ...
    this.loadList();
    //180000 -- 3 minutos inicia y en 3 minutos vuelve a llamar

    let timer_ = timer(180000, 180000)
      .pipe(timeInterval())
      .subscribe((t) => {
    ...
    /*     this.timer.pipe(takeUntil(this.destroy$)).subscribe((t) => {
  ...
  async editarRegistro(cond: string) {
    ...
    if (cond == 'operacioncobro') {
      console.log('ausente');
      this.router.navigate(['/recepcion/operacioncobro'], {
  ...
  loadList() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        console.log(this.popItemAgenda);
        this.miServico.getHorarioTurnoTodos(this.popItemAgenda).subscribe(
          (resp) => {
            if (resp[0]) {
  ...
  loadHistoriaPaciente() {
    try {
      this.miServico.getHistoriaTurno(this.popItemPaciente.id).subscribe(
        (resp) => {
          if (resp[0]) {
            this.realizarFiltroBusqueda(resp);
            this.agendaTurno = resp;
  ...
  loadAgendaCancelada() {
    this.loading = true;
    try {
      this.miServico.getTurnoCancelado().subscribe(
        (resp) => {
          if (resp[0]) {
            this.realizarFiltroBusqueda(resp);
            this.agendaTurno = resp;
  ...
  cancelarTurno() {
    ...
    try {
      ...
      this.miServico
        .cancelarTurno(this.popItemAgenda.agenda_dia_horario_atencion_id)
        .subscribe(
          (resp) => {
            this.loading = false;
  ...
  loadListByMedico() {
    ...
    if (this._fechaHoy != '') {
      ...
      this.popItemAgenda.usuario_id = this.popItemMedicoObraSocial.usuario_id;
      try {
        this.miServico.getHorarioTurnoMedico(this.popItemAgenda).subscribe(
          (resp) => {
            //  console.log(resp);
  ...
  ActualizarTurnoLlamando() {
    try {
      this.miServico
        .ActualizarTurnoLlamando(
          this.popItemAgenda.paciente_id,
          this.popItemAgenda.usuario_id,
  ...
  actualizarTurno() {
    ...
    try {
      this.miServico
        .putItem(
          this.popItemAgenda,
          this.popItemAgenda.agenda_dia_horario_atencion_id
        )
        .subscribe(
          (resp) => {
            console.log(resp);
  ...
  verDetalle(agendaTurno: any) {
    ...
    liquidacion = new Liquidacion(
      agendaTurno['operacion_cobro_id'],
      '',
      '',
      '',
      '',
  ...
  sumarPresente() {
    ...
    if (this.agendaTurnos) {
      for (i = 0; i < this.agendaTurnos.length; i++) {
        if (
          this.agendaTurnos[i]['presente'] !== null &&
          this.agendaTurnos[i]['presente'] !== '2099-12-31 00:00:00' &&
          this.agendaTurno[i]['llegada'] === '2099-12-31 00:00:00' &&
          this.agendaTurno[i]['estado'] !== 'LLAMANDO' &&
          this.agendaTurno[i]['estado'] !== 'ATENDIDO'
        ) {
        ...
        if (
          this.agendaTurnos[i]['presente'] !== null &&
          this.agendaTurnos[i]['estado'] === 'DERIVADO'
        ) {
        ...
        if (
          this.agendaTurnos[i]['presente'] !== null &&
          this.agendaTurnos[i]['estado'] === 'ASESORAMIENTO'
        ) {
  ...
  realizarFiltroBusqueda(resp: any[]) {
    ...
    resp.forEach((element) => {
      ...
      this._paciente_obra_social_nombre.push(
        element['paciente_obra_social_nombre']
      );
    ...
    console.log(this._nombreyapellido);
    this._paciente_obra_social_nombre = this.filter.filterArray(
      this._paciente_obra_social_nombre
    );
