...
export class PopupTurnoUsuarioObraSocialComponent implements OnInit {
  ...
  constructor(
    private medicoObraSocialServicio: MedicoObraSocialService,
  ...
  loadListMedico() {
    ...
    try {
      this.medicoServicio.getItems().subscribe(
        (resp) => {
          this.medico = resp;
          this.loading = false;
          console.log(resp);
 ...
 */
  ...
  FindByListObraSocial() {
    ...
    try {
      this.medicoObraSocialServicio
        .getItemByObraSocial(this.selectedItemObraSocial.id)
        .subscribe(
          (resp) => {
            if (resp.length) {
              this.medicoObraSocial = resp;
