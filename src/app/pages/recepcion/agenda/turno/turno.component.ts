...
export class TurnoComponent implements OnInit {
    ...
    ref.onClose.subscribe((PopupObraSocialComponent: ObraSocial) => {
      if (PopupObraSocialComponent) {
        ...
        this.formPaciente.patchValue({
          obra_social_nombre_adicional_id: PopupObraSocialComponent.id,
        });
        this.formPaciente.patchValue({
          obra_social_nombre_adicional: PopupObraSocialComponent.nombre,
        });
      }
    });
  }
  ...
  loadListObraSocial() {
    ...
    try {
      this.obraSocialServicio.getItems().subscribe(
        (resp) => {
      ...
      this.miServicoPaciente.getItems('paciente.dni', dni).subscribe(
        (resp) => {
          ...
          this.AgendaForm.patchValue({
            medico_nombre: _agenda.nombreyapellido,
          });
          this.formPaciente.patchValue({
            fecha_nacimiento: _agenda.paciente_fecha_nacimiento,
          });
          this.formPaciente.patchValue({
            obra_social_nombre_adicional_id: _agenda.obra_social_alterantiva_id,
          });
          this.formPaciente.patchValue({
            obra_social_nombre_adicional:
              _agenda.obra_social_alterantiva_nombre,
          });
          console.log(_agenda);
  ...
  guardarObservacion() {
    ...
    if (!this.selectedMedicoEfector['nombreyapellido']) {
      ...
      selectedMedicoEfector_ =
        'MEDICO:  ' + this.selectedMedicoEfector['nombreyapellido'];
    }
  ...
  cargarAgendaInicio(agenda: AgendaTurno) {
    ...
    this.popItemPaciente.obra_social_nombre =
      agenda.paciente_obra_social_nombre;
    ...
    this.formPaciente.patchValue({
      obra_social_nombre: agenda.paciente_obra_social_nombre,
    });
    this.usuario_id = agenda.usuario_id;
    this.AgendaForm.patchValue({ medico_nombre: agenda.nombreyapellido });
    this.formPaciente.patchValue({
      fecha_nacimiento: agenda.paciente_fecha_nacimiento,
    });
    this.calendarioBloqueo();
  }
  ...
  loadListTurno() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        ...
        this.miServico
          .getHorarioTurnoDisponibleByUsuario(
            this._fechaHoy,
            '1',
            this.usuario_id
          )
      ...
      this.throwAlert(
        'error',
        'Error: ' + 404 + '  No se selecciono una fecha',
        'Error en la fecha',
        '400'
      );
    }
  ...
  loadListTurnoTodos() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        this.miServico
          .getAgendaByMedicoDiaTodosEstado(this._fechaHoy, '1', this.usuario_id)
          .subscribe(
            (resp) => {
              this.agendaTurno = resp;
              this.loading = false;
  ...
  loadSobreTurno() {
    ...
    if (this._fechaHoy != '') {
      ...
      try {
        this.miServico
          .getHorarioTurnoDisponibleByUsuarioTodos(
            this._fechaHoy,
            '1',
            this.usuario_id
          )
  ...
  buscarUsuarioObraSocial() {
    ...
    ref.onClose.subscribe(
      (PopupTurnoUsuarioObraSocialComponent: MedicoObraSocial) => {
        if (PopupTurnoUsuarioObraSocialComponent) {
          ...
          this.AgendaForm.patchValue({
            medico_nombre:
              PopupTurnoUsuarioObraSocialComponent.apellido +
              ' ' +
              PopupTurnoUsuarioObraSocialComponent.nombre,
          });
          this.apellido = PopupTurnoUsuarioObraSocialComponent.apellido;
          this.nombre = PopupTurnoUsuarioObraSocialComponent.nombre;
          this.obra_social_nombre =
            PopupTurnoUsuarioObraSocialComponent.obra_social_nombre;
          this.medico_nombre =
            PopupTurnoUsuarioObraSocialComponent.apellido +
            ' ' +
            PopupTurnoUsuarioObraSocialComponent.nombre;
          this.usuario_medico_factura_id =
            PopupTurnoUsuarioObraSocialComponent.usuario_id;
          this.medico_nombre_factura =
            PopupTurnoUsuarioObraSocialComponent.apellido +
            ' ' +
            PopupTurnoUsuarioObraSocialComponent.nombre;
  ...
  getUsuarioMedico() {
    this.loadingAccion = true;
    try {
      this.miUserServico.getItems().subscribe(
        (resp) => {
          this.usuarios = resp;
          console.log(resp);
  ...
  buscarPaciente() {
    ...
    ref.onClose.subscribe((PopupPacienteObrasocialComponent: Paciente) => {
      if (PopupPacienteObrasocialComponent) {
        ...
        this.formPaciente.patchValue({
          obra_social_nombre_adicional_id:
            PopupPacienteObrasocialComponent.obra_social_id,
        });
    ...
    ref.onClose.subscribe((PopupPacienteNuevoComponent: Paciente) => {
      if (PopupPacienteNuevoComponent) {
        ...
        if (this.nuevoItemPaciente()) {
          ...
          this.formPaciente.patchValue({
            paciente_id: this.popItemPaciente.id,
          });
          this.formPaciente.patchValue({
            obra_social_nombre: this.popItemPaciente.obra_social_nombre,
          });
          console.log(this.formPaciente.value);
          console.log(PopupPacienteObrasocialComponent);
        }
  ...
  nuevoItemPaciente() {
    this.loadingAccion = true;
    try {
      this.miServicoPaciente.postItem(this.popItemPaciente).subscribe(
        (resp) => {
  ...
  editarItemPaciente() {
    ...
    try {
      console.log(this.popItemPaciente);
      this.miServicoPaciente
        .putItem(this.popItemPaciente, this.popItemPaciente.id)
        .subscribe(
          (resp) => {
            this.elementoPaciente = resp;
            console.log(this.elementoPaciente);
            ...
            this.throwAlert(
              'success',
              'Se modificó agendo el paciente ' + this.popItemPaciente.apellido,
              '',
              '200'
            );
            this.resultSave = true;
    ...
    if (this.motivoObservaciones.length > 0) {
      ...
      if (this.motivoObservaciones[3] === undefined) {
        ...
        if (this.motivoObservaciones[0] === 'SIN ESTUDIOS') {
          ...
          this.observaciones =
            this.motivoObservaciones[1] + '     ' + this.motivoObservaciones[0];
          console.log(this.observaciones);
        }
      } else {
        if (this.motivoObservaciones[0] === 'SIN ESTUDIOS') {
          ...
          this.observaciones =
            this.motivoObservaciones[1] +
            '       ' +
            this.motivoObservaciones[3];
        } else {
          console.log(this.motivoObservaciones[0]);
          this.observaciones =
            this.motivoObservaciones[1] +
            '       ' +
            this.motivoObservaciones[0] +
            '       ' +
            this.motivoObservaciones[3];
        }
    ...
    if (this.elemento.paciente_id) {
      ...
      try {
        ...
        if (this.elemento.paciente_id !== '0') {
          ...
          this.throwAlert(
            'error',
            'El id del paciente es 0, verifique los datos del paciente ',
            'Datos de paciente invalidos',
            '500'
          );
        }
        if (this.elemento.observacion !== undefined) {
          ...
          this.throwAlert(
            'warning',
            'la observacion no fue definida correctamente, verifique los datos',
            'Observación con datos undefined',
            '500'
          );
        }
        if (datos_validos) {
          ...
          this.elemento.obra_social_nombre_adicional_id =
            this.formPaciente.value.obra_social_nombre_adicional_id;
          this.miServico.agendarTurno(this.elemento).subscribe(
            (resp) => {
              this.loadingAccion = false;
              this.throwAlert(
                'success',
                'Se  agendo el paciente ' +
                  this.popItemPaciente.apellido +
                  ' ' +
                  this.popItemPaciente.nombre +
                  ' con el medico ' +
                  event.nombreyapellido,
                '',
                '200'
              );
              ...
              swal({
                ...
                text:
                  '¿Desea dar otro turno para el paciente' +
                  this.popItemPaciente.apellido +
                  ' ' +
                  this.popItemPaciente.nombre +
                  ' con el medico ' +
  ...
  colorRow(fecha: Date) {
    ...
    let t2: string;
    t2 =
      this.leftPad(String(fecha['day']), 2, '0') +
      '/' +
      this.leftPad(String(fecha['month'] + 1), 2, '0') +
      '/' +
      this.leftPad(String(fecha['year']), 2, '0');
