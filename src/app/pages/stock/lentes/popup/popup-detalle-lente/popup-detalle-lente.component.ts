...
export class PopupDetalleLenteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.config.data) {
      console.log(
        this.listaestado.find((x) => x.name === this.config.data.estado)
      );
      this.es_nuevo = false;
      this.selectedForma = this.listaestado.find(
        (x) => x.name === this.config.data.estado
      );
      this._fechaHoy = formatDate(
        this.config.data.fecha_vencimiento,
        'yyyy-MM-dd',
        'es-AR'
      );
  ...
  guardarDatos() {
    ...
    this._fechaHoy = formatDate(
      this.DataForm.value.fecha_vencimiento,
      'yyyy-MM-dd',
      'es-AR'
    );
    ...
    if (this.es_nuevo) {
      ...
      console.log(this.DataForm.value);
      try {
        this.miServicio.postLente(this.DataForm.value).subscribe(
          (resp) => {
            this.loading = false;
            this.throwAlert(
              'success',
              'Se inserto el registro con éxito',
              '',
              ''
            );
            this.ref.close(resp);
      ...
      try {
        ...
        console.log(this.element);
        this.miServicio.putLente(this.element, this.config.data.id).subscribe(
          (resp) => {
            this.loading = false;
            this.throwAlert(
              'success',
