...
export class AltaLenteComponent implements OnInit {
  ...
  public exportarExcel() {
    ...
    this.liquidacionService.exportAsExcelFile(
      this.selecteditems,
      'listado_existencia_lentes' + fecha_impresion
    );
  }
  ...
  buscarLentesTodos() {
    ...
    try {
      this.miServicio.getLentesTodosByDate(td, th).subscribe(
        (resp) => {
  ...
  buscarLentesFecha() {
    ...
    try {
      this.miServicio.getLentesCirugiaByDates(td, th).subscribe(
        (resp) => {
          this.realizarFiltroBusqueda(resp);
          this.elementos = resp;
          console.log(resp);
  ...
  buscarLentesBaja() {
    ...
    try {
      this.miServicio.getLentesCirugiaByDatesAndBaja(td, th, 'SI').subscribe(
        (resp) => {
          this.realizarFiltroBusqueda(resp);
          this.elementos = resp;
          console.log(resp);
  ...
  loadList() {
    this.loading = true;
    try {
      this.miServicio.getLentesSinUso('NO').subscribe(
        (resp) => {
          this.realizarFiltroBusqueda(resp);
