...
export class ConfeccionFacturaComponent implements OnInit {
  ...
  actualizarRegistros() {
    if (this.selecteditems) {
      ...
      console.log(this.selecteditems);
      try {
        this.miServicio.DistribuirOperacionCobro(this.selecteditems).subscribe(
          (resp) => {
            swal({
              toast: false,
              type: 'success',
  ...
  generarPdfListadoCirugiaTodos() {
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                console.log(
                  this.elementosPreFactura[j]['complejidad'] +
                    ' cirugia ' +
                    this.elementosPreFactura[j]['descripcion']
                );
              } else {
                console.log('coseguro honorarios');
                let t_hono = this.cp.transform(
                  (this.elementosPreFactura[j][
                    'operacion_cobro_distribucion_total'
                  ] *
                    20) /
    ...
    for (i = 0; i < filteredArr.length; i++) {
      if (filteredArr[i]['valor_facturado']['complejidad'] === 2) {
        ...
        total_facturado =
          total_facturado +
          Number(filteredArr[i]['valor_facturado']) +
          Number(filteredArr[i]['categorizacion']);
