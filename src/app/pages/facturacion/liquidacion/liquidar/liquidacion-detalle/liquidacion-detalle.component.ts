...
export class LiquidacionDetalleComponent implements OnInit {
  ...
  accion(event: any, overlaypanel: OverlayPanel, elementos: Liquidacion) {
    if (elementos) {
  ...
  verDetalle() {
    ...
    ref.onClose.subscribe(
      (PopupOperacionCobroPresentacionComponent: ObraSocial) => {
  ...
  editarDistribucion() {
    ...
    ref.onClose.subscribe(
      (PopupOperacionCobroEditarDistribucionComponent: any) => {
  ...
  clonarDistribucion() {
    ...
    try {
      this.miServicio.clonarLiquidacion(this.selecteditems).subscribe(
        (resp) => {
          console.log(resp);
          this.loadlist();
          this.loading = false;
      ...
      this.miServicio.getListadoPreFactura(this.selecteditems).subscribe(
        (resp) => {
          ...
          resultado.forEach((element) => {
            resp[i]['total_con_categoria'] = this.cp.transform(
              Number(element['valor_facturado']) +
                Number(element['categorizacion']),
              '',
              'symbol-narrow',
              '1.2-2'
            );
  ...
  loadPresentacionCirugiaTodos() {
    ...
    try {
      this.miServicio.getListadoPreFacturaCirugia(this.selecteditems).subscribe(
        (resp) => {
          let i: number = 0;
          let resultado = resp;
  ...
  loadPresentacionCirugiaCoseguroTodos() {
    ...
    try {
      this.miServicio
        .getListadoPreFacturaCirugiaCoseguro(this.selecteditems)
        .subscribe(
          (resp) => {
            let i: number = 0;
            let resultado = resp;
  ...
  generarFactura() {
    ...
    try {
      this.facturacionService.CrearFacturaA('24').subscribe(
        (resp) => {
  ...
  generarTxt() {
    ...
    try {
      this.miServicio.generarTxt(this.selecteditems).subscribe(
        (resp) => {
          this.throwAlert('success', 'Se generó el archivo con éxito', '', '');
          this.loading = false;
          console.log(resp);
        ...
        (resp) => {
          ...
          for (i = 0; i < this.elementosPreFactura.length; i++) {
            ...
            for (j = 0; j < this.elementosPreFactura.length; j++) {
              if (
                this.elementosPreFactura[j]['convenio_os_pmo_id'] === practica
              ) {
                if (
                  this.elementosPreFactura[j]['obra_social_practica_nombre'] ===
                    'HONORARIOS' &&
                  this.elementosPreFactura[j]['complejidad'] !== 2
                ) {
                  if (
                ...
                if (
                  ...
                  if (
                    ...
                    if (
                      ...
                      this.elementosPreFactura[i]['gastos'] =
                        this.elementosPreFactura[j][
                          'operacion_cobro_distribucion_total'
                        ];
                    } else {
                      console.log('coseguro');
                      this.elementosPreFactura[i]['gastos'] = this.cp.transform(
                        (this.elementosPreFactura[j][
                          'operacion_cobro_distribucion_total'
                        ] *
                          20) /
                ...
                if (this.elementosPreFactura[j]['complejidad'] === 2) {
                  this.elementosPreFactura[i]['gastos'] =
                    this.elementosPreFactura[j]['valor_facturado'];
          ...
          const filteredArr = this.elementosPreFactura.reduce(
            (acc, current) => {
              const x = acc.find(
                (item) =>
                  item['operacion_cobro_practica_id'] ===
                  current['operacion_cobro_practica_id']
              );
              if (!x) {
          ...
          try {
            this.miServicio.generarTxtCirugia(filteredArr).subscribe(
              (resp) => {
                this.throwAlert(
                  'success',
                  'Se generó el archivo con éxito',
                  '',
                  ''
                );
                this.loading = false;
  ...
  public exportarExcelRegistro() {
    ...
    this.selecteditems.forEach((element) => {
      ...
      seleccionados['fecha_desde'] = formatDate(
        element['fecha_desde'],
        'dd/MM/yyy',
        'es-Ar'
      );
      seleccionados['fecha_hasta'] = formatDate(
        element['fecha_hasta'],
        'dd/MM/yyy',
        'es-Ar'
      );
  ...
  sumarValoresSeleccionados(vals: any) {
    ...
    for (i = 0; i < vals.length; i++) {
      ...
      this.total_seleccionado =
        this.total_seleccionado + Number(vals[i]['total']);
    }
  }
  ...
  desafectarPresentacion() {
    ...
    try {
      this.practicaService
        .desafectarPresentacion(this.selecteditemRegistro['id'])
        .subscribe(
          (resp) => {
            this.throwAlert(
              'success',
              'Se desafecto el registro  con éxito',
              '',
              ''
            );
            this.loading = false;
  ...
  generarPdfListadoMedico() {
    ...
    if (this.selecteditems) {
      ...
      doc.autoTable(this.columnsListadoTodos, this.elementosPreFactura, {
        ...
        columnStyles: {
          paciente_nombre: { columnWidth: 30 },
          numero_afiliado: { columnWidth: 15, fontSize: 5 },
          codigo: { columnWidth: 15, fontSize: 5 },
          ...
          matricula: { columnWidth: 15 },
          medico_nombre: { columnWidth: 30 },
        },
        addPageContent: (data) => {
  ...
  generarPdfListadoTodosConCategoria() {
    ...
    for (i = 0; i < this.elementosPreFactura.length; i++) {
      ...
      total_facturado =
        ...
        Number(this.elementosPreFactura[i]['valor_facturado']);
      total_categoria =
        total_categoria + Number(this.elementosPreFactura[i]['categorizacion']);
      console.log(this.elementosPreFactura[i]['cantidad']);
    }
    total_cantidad_impresion = this.dp.transform(total_cantidad, '1.0-0');
    if (this.selecteditems) {
      ...
      doc.text(
        ...
        finalY + 8,
        'Categoria : ' +
          this.cp.transform(total_categoria, '', 'symbol-narrow', '1.2-2')
      );
      doc.text(
        pageWidth - 35,
        finalY + 8,
        'Total : ' +
          this.cp.transform(
            total_facturado + total_categoria,
            '',
            'symbol-narrow',
            '1.2-2'
          )
  ...
  generarPdfListadoCirugiaTodos() {
      ...
      for (j = 0; j < this.elementosPreFactura.length; j++) {
        if (this.elementosPreFactura[j]['convenio_os_pmo_id'] === practica) {
          if (
            this.elementosPreFactura[j]['obra_social_practica_nombre'] ===
              'HONORARIOS' &&
            Number(this.elementosPreFactura[j]['complejidad']) !== 2
          ) {
            if (
              this.elementosPreFactura[j][
                'operacion_cobro_distribucion_total'
              ...
              if (
                ...
                this.elementosPreFactura[i]['honorarios'] = String(
                  (this.elementosPreFactura[j][
                    'operacion_cobro_distribucion_total'
                  ] *
                    20) /
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                this.elementosPreFactura[i]['categoria'] = this.cp.transform(
                  0,
                  '',
                  '',
                  '1.2-2'
          ...
          if (_complejidad_original === 4) {
            if (
              ...
              this.elementosPreFactura[i]['gastos'] =
                this.elementosPreFactura[i]['valor_facturado'];
            } else {
              let gasto: string = String(
                this.elementosPreFactura[j]['valor_facturado']
              );
    ...
    for (i = 0; i < filteredArr.length; i++) {
      ...
      if (filteredArr[i]['complejidad'] === 2) {
        ...
        if (_complejidad === 3) {
          total_honorario =
            total_honorario +
            Number(filteredArr[i]['honorarios']) +
            Number(filteredArr[i]['categorizacion']);
          filteredArr[i]['honorarios'] =
            Number(filteredArr[i]['honorarios']) +
            Number(filteredArr[i]['categorizacion']);
          total_gastos = total_gastos + Number(filteredArr[i]['gastos']);
        }
        ...
        if (
          ...
          filteredArr[i]['gastos'] === NaN
        ) {
          filteredArr[i]['gastos'] = 0;
        }
        if (
          filteredArr[i]['honorarios'] === undefined ||
          filteredArr[i]['honorarios'] === NaN
        ) {
          filteredArr[i]['honorarios'] = 0;
        }
        if (
          filteredArr[i]['categorizacion'] === undefined ||
          filteredArr[i]['categorizacion'] === NaN
        ) {
          filteredArr[i]['categorizacion'] = 0;
        }
         ...
         *
         */
        filteredArr[i]['valor_facturado'] = this.cp.transform(
          Number(filteredArr[i]['honorarios']) +
            Number(filteredArr[i]['gastos']),
          '',
          'symbol-narrow',
          '1.2-2'
        );
        total_facturado =
          ...
          Number(filteredArr[i]['honorarios']) +
          Number(filteredArr[i]['gastos']);
        filteredArr[i]['gastos'] = this.cp.transform(
          Number(filteredArr[i]['gastos']),
          '',
          'symbol-narrow',
          '1.2-2'
        );
        filteredArr[i]['honorarios'] = this.cp.transform(
          Number(filteredArr[i]['honorarios']),
          '',
          'symbol-narrow',
          '1.2-2'
        );
        filteredArr[i]['categorizacion'] = this.cp.transform(
          Number(filteredArr[i]['categorizacion']),
          '',
          'symbol-narrow',
          '1.2-2'
    ...
    if (this.selecteditems) {
      ...
      doc.text(
        ...
        finalY + 8,
        'Honorarios : ' +
          this.cp.transform(total_honorario, '', 'symbol-narrow', '1.2-2')
      );
      doc.text(
        pageWidth - 90,
        finalY + 8,
        'Gastos : ' +
          this.cp.transform(total_gastos, '', 'symbol-narrow', '1.2-2')
      );
      doc.text(
        pageWidth - 50,
        finalY + 8,
  ...
  generarPdfListadoMedicoIVA() {
    ...
    if (this.selecteditems) {
      ...
      doc.setFontSize(9);
      doc.text(
        this.elementosPreFactura[0]['liquidacion_nombreyapellido'],
        60,
        10,
        null,
        null,
      ...
      doc.text(
        ...
        finalY + 8,
        'Importe : ' +
          this.cp.transform(total_sin_iva, '', 'symbol-narrow', '1.2-2')
      );
      doc.text(
        pageWidth - 80,
        finalY + 8,
