...
import { MedicoObraSocial } from '../../../../../../../vision-frontend-v2/src/app/models/medico-obrasocial.model';
...
export class LiquidarComponent implements OnInit {
  ...
  generarLiquidacion() {
    if (this.selecteditems) {
      ...
      this.loading = true;
      liquidacionGenerada = new LiquidacionGenerada(
        String(this.liquidacion_nro),
        this.detalle,
        formatDate(this.fechaLiquidacion, 'dd/MM/yyyy', 'en'),
        'LIQ'
      );
      ...
      try {
        this.miServicio.generarLiquidacionNumero(liquidacionGenerada).subscribe(
          (resp) => {
            swal({
              toast: false,
              type: 'success',
  ...
  generarLiquidacionExpedientes(liquidacion: any[]) {
    if (this.selecteditems) {
      ...
      try {
        this.miServicio
          .LiquidarExpedientes(this.selecteditems, liquidacion[0].numero)
          .subscribe(
            (resp) => {
              swal({
                toast: false,
  ...
  verDetalleTodos() {
    ...
    ref.onClose.subscribe(
      (PopupOperacionCobroDistribucionMultipleComponentComponent: any) => {
  ...
  verDistribucion(elementos: Liquidacion, consulta: string) {
    ...
    ref.onClose.subscribe(
      (PopupOperacionCobroDistribucionDetalleComponent: any) => {
  ...
  loadlist() {
    ...
    try {
      this.miServicio.getLiquidacionDetalle('DIS').subscribe(
        (resp) => {
          if (resp[0]) {
            this.elementos = resp;
  ...
  loadDistribucionTodos() {
    ...
    try {
      this.miServicio.GetDistribucionByExpediente(this.selecteditems).subscribe(
        (resp) => {
          let i: number = 0;
          let resultado = resp;
  ...
  loadDistribucionMedico() {
    ...
    try {
      this.miServicio.GetDistribucionByMedico(this.selecteditems).subscribe(
        (resp) => {
          let i: number = 0;
          let resultado = resp;
          resultado.forEach((element) => {
  ...
  loadDistribucionMedicoDetalle() {
    ...
    try {
      this.miServicio
        .GetDistribucionByMedicoDetalle(this.selecteditems)
        .subscribe(
          (resp) => {
            let i: number = 0;
            let resultado = resp;
