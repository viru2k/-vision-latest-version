...
export class OtrasAccionesComponent implements OnInit {
  ...
  generarPDF() {
    ...
    h2c(document.querySelector('#qr_code_new')).then((canvas) => {
      ...
      this._fecha = formatDate(
        this.elementosPDF[0]['fecha'],
        'dd/MM/yyyy',
        'en'
      );
      ...
      doc.setFontSize(6);
      doc.text(
        'COD. ' + this.elementosPDF[0]['comprobante_codigo'],
        pageWidth / 2 - 4.5,
        21
      );
      doc.addImage(
      ...
      doc.text(
        'Comprobante: ' +
          this.padLeft(this.elementosPDF[0]['punto_vta'], '0', 4) +
          ' - ' +
          this.padLeft(this.elementosPDF[0]['factura_numero'], '0', 8),
        pageWidth / 2 + 20,
        20
      );
      doc.setFontSize(9);
      ...
      doc.text(
        'C.U.I.T.: ' + this.elementosPDF[0]['cuit'],
        pageWidth / 2 + 20,
        30
      );
      doc.text(
        'Ingresos brutos: ' + this.elementosPDF[0]['ing_brutos'],
        pageWidth / 2 + 20,
        35
      );
      doc.text(
        'Inicio de actividades: ' + this.elementosPDF[0]['fecha_alta_afip'],
        pageWidth / 2 + 20,
        40
      );
      doc.setLineWidth(0.4);
      ...
      doc.text(
        this.elementosPDF[0]['factura_comprobante_descripcion'] +
          ' : ' +
          this.elementosPDF[0]['factura_documento'],
        15,
        73
      );
      doc.text('Cliente: ' + this.elementosPDF[0]['factura_cliente'], 15, 78);
      doc.text(
        'Condición frente al IVA: ' + this.elementosPDF[0]['categoria_iva'],
        15,
        83
      ); // PENDIENTE
      ...
      doc.text(
        'Subtotal: ' +
          this.cp.transform(
            this.elementosPDF[0]['importe_gravado'],
            '',
            'symbol-narrow',
            '1.2-2'
          ),
      ...
      if (
        this.elementosPDF[0]['factura_comprobante_id'] === 6 ||
        this.elementosPDF[0]['factura_comprobante_id'] === 11
      ) {
      } else {
        doc.text(
          'Imp. IVA: ' +
            this.cp.transform(
              this.elementosPDF[0]['importe_iva'],
              '',
              'symbol-narrow',
              '1.2-2'
            ),
      ...
      doc.text(
        'Total: ' +
          this.cp.transform(
            this.elementosPDF[0]['importe_total'],
            '',
            'symbol-narrow',
            '1.2-2'
          ),
      ...
      'Fecha de vencimiento de CAE: ' +
        formatDate(this.elementosPDF[0]['cae_vto'], 'dd/MM/yyyy', 'en'),
      pageWidth / 2 + 20,
      pageHeight - 5
    ); */
      doc.setFontStyle('normal');
      ...
      doc.addImage(imgData, 'PNG', pageWidth - 34, 68);
      window.open(doc.output('bloburl'));
      this.elementosPDF = [];
      this.CAE = null;
    });
