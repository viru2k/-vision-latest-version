...
export class PopupCombinadaComponent implements OnInit {
  ...
  constructor(
    private miServico: PracticaDistribucionService,
    ...
    this.cols = [
      ...
      {
        field: 'obra_social_practica_nombre',
        header: 'Descripcion',
        width: '30%',
      },
  ...
  agregarRegistro() {
    ...
    ref.onClose.subscribe(
      (PopupCombinadaItemComponent: PracticaDistribucionRegistro) => {
        if (PopupCombinadaItemComponent) {
          this.elementoRegistro = PopupCombinadaItemComponent;
          console.log(this.elementoRegistro);
          this.item = new PracticaDistribucionRegistro(
            '0',
            this._id,
            this.elementoRegistro.practica_distribucion_id,
            this.elementoRegistro.porcentaje,
            this.elementoRegistro.valor,
            this.elementoRegistro.total,
            this.elementoRegistro.codigo,
            this.elementoRegistro.complejidad,
            this.elementoRegistro.es_habilitada,
            this.elementoRegistro.obra_social_id,
            this.elementoRegistro.obra_social_nombre,
            this.elementoRegistro.pmo_descripcion,
            this.elementoRegistro.pmo_id
          );
  ...
  calcularTotal() {
    ...
    if (this.elementos) {
      for (let entry of this.elementos) {
        ...
        this.totalFacturado =
          this.totalFacturado + +entry['practica_distribucion_total'];
      }
    }
  }
  ...
  loadList() {
    ...
    this.loading = true;
    try {
      this.miServico.getItems(this._id).subscribe(
        (resp) => {
          if (resp[0]) {
            this.elementos = resp;
  ...
  eliminarRegistro(row) {
    ...
    try {
      this.miServico.delItem(row.id).subscribe(
        (resp) => {
          this.elemento = resp;
  ...
  nuevoItem() {
    try {
      this.miServico.postItem(this.item).subscribe(
        (resp) => {
          this.elemento = resp;
          console.log(this.elemento);
          this.loading = false;
