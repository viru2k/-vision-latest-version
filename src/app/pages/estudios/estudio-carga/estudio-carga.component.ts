...
export class EstudioCargaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.paciente != null) {
      ...
      const timeDiff = Math.abs(
        Date.now() - new Date(this.paciente.paciente_fecha_nacimiento).getTime()
      );
    ...
    ref.onClose.subscribe((PopupPacienteObrasocialComponent: Paciente) => {
      if (PopupPacienteObrasocialComponent) {
        ...
        let timeDiff = Math.abs(
          Date.now() - new Date(this.popItemPaciente.fecha_nacimiento).getTime()
        );
  ...
  buscarMedico() {
    ...
    ref.onClose.subscribe((PopupMedicoComponent: MedicoObraSocial) => {
      if (PopupMedicoComponent) {
        ...
        this.formPaciente.patchValue({
          medico_id: this.popItemMedico.usuario_id,
        });
        this.formPaciente.patchValue({
          medico_nombre:
  ...
  onUpload(event) {
    ...
    for (let file of event.files) {
      this.estudio = new Estudios(
        '',
        this.formPaciente.value.selectedEstudio['name'],
        this.formPaciente.value.paciente_id,
        this.formPaciente.value.medico_id,
        this._fechaHoy,
        this.userData['id'],
        file.name,
        URL_ARCHIVO,
        ...
        this.formPaciente.value.SINTOMAS_SIGNOS
      );
  ...
  uploadEstudioDatos(datos: Estudios[]) {
    this.loading = true;
    try {
      this.cirugiaService.uploadEstudioDatos(datos).subscribe(
        (resp) => {
          // this.elementos = resp;
          console.log(resp);
          this.throwAlert(
            'success',
            'Se subieron los archivos con éxito',
            '',
            ''
          );
